...
class GradientReversal(nn.Module):
    """Reverse gradients so a nuisance classifier removes nuisance signal."""
    def __init__(self, strength: float = 1.0) -> None:
        super().__init__()
        self.strength = strength

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return GradientReversalFn.apply(x, self.strength)
...
class ContextEncoder(nn.Module):
    def __init__(self, n_species: int, n_assays: int, hidden_dim: int = 64) -> None:
        ...
        self.net = nn.Sequential(nn.Linear(24, hidden_dim), nn.LayerNorm(hidden_dim), nn.GELU(), nn.Linear(hidden_dim, hidden_dim))
...
class CardiLearnLarge(nn.Module):
    ...
    def __init__(self, n_genes: int, n_species: int, n_assays: int, n_cell_types: int,
                 ...
                 private_dim: int = 256, dropout: float = 0.1, species_grl: float = 0.25) -> None:
        super().__init__()
        if gene_dim % n_heads != 0:
            raise ValueError("gene_dim must be divisible by n_heads")
        if n_genes < 5000:
        ...
        self.species_adversary = nn.Sequential(nn.Linear(shared_dim, 256), nn.GELU(), nn.Linear(256, n_species))
        self.grl = GradientReversal(species_grl)
    ...
    def encode(self, x: torch.Tensor, species: torch.Tensor, assay: torch.Tensor):
        if x.ndim != 2 or x.shape[1] != self.n_genes:
            raise ValueError(f"Expected x with shape [batch, {self.n_genes}], got {tuple(x.shape)}")
        gene_tokens = self.expression_projection(self.count_input(x).unsqueeze(-1)) + self.gene_embedding.unsqueeze(0)
        queries = self.program_queries.unsqueeze(0).expand(x.shape[0], -1, -1)
    ...
    def decode_counts(self, z_shared: torch.Tensor, z_private: torch.Tensor, species: torch.Tensor, assay: torch.Tensor, library_size: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if torch.any(library_size <= 0):
            raise ValueError("library_size must be positive")
        hidden = self.decoder(torch.cat([z_shared, z_private, self.context(species, assay)], dim=-1))
        proportions = torch.softmax(self.mu_head(hidden), dim=-1)
        mu = proportions * library_size.reshape(-1, 1).to(proportions.dtype)
